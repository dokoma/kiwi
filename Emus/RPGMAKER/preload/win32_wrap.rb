...
module Graphics
	class << self
		...
		alias_method(:w32wr_width, :width) if Graphics.respond_to? :width
		alias_method(:w32wr_height, :height) if Graphics.respond_to? :height
	end
end
...
def input_mod_keystate(key)
	val = 0
	
	# See GetAsyncKeyState docs
	val |= (1 << 00) if Input.trigger?(key)
	val |= (1 << 15) if Input.press?(key)
...
def common_keystate(vkey)
	...
	if vkey_name == :LBUTTON
		return input_mod_keystate(Input::MOUSELEFT)
	elsif vkey_name == :RBUTTON
		return input_mod_keystate(Input::MOUSERIGHT)
	elsif vkey_name == :MBUTTON
		return input_mod_keystate(Input::MOUSEMIDDLE)
	end
...
#				return common_keystate(vkey[0]) == 1 ? PRESSED_BIT : 0
		...
		class GetClientRect
			def initialize
				@is_vx = Graphics.respond_to? :width
			end
			def call(args)
				return 0 if args[0] != HWND
				rect = [0, 0, 640, 480]
				if @is_vx
					rect[2] = Graphics.w32wr_width
					rect[3] = Graphics.w32wr_height
				end
				# XXX Something is shadowing the real Graphics.width/height...
#				rect[2], rect[3] = 640, 480
